...
def storeRecordings(items):
    ...
    with open(name, "w", newline="") as writeRecordings:
        for id in items:
            recordId = id["id"]
            hostEmail = id["ownerEmail"]
            print(recordId, hostEmail)
            writer = csv.writer(writeRecordings, delimiter=",")
            writer.writerow((recordId, hostEmail))


def list_recordings(headers, site_url, weeks):
    # assign current date and time in ISO8601 format to variable to_time
    ...
    x = int(weeks) * 7  # replace with the desired number of days
    from_time_d = to_time - datetime.timedelta(days=x)
    from_time = from_time_d.replace(microsecond=0, tzinfo=None).isoformat()
    end_time = to_time.replace(microsecond=0, tzinfo=None).isoformat()
    # print(to_time)
    # print(from_time)
    more = True
    count = 0
    while more:
        try:
            url = "https://webexapis.com/v1/admin/convergedRecordings?siteUrl={0}&max=100&from={1}&to={2}".format(
                site_url, from_time, end_time
            )
            print("URL: " + url)
            # print(headers)
            ...
            if response.status_code == 401:
                ...
                # TODO: Download also the transcript of each recording
